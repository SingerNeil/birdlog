...
- 自动同步：新增、编辑、删除会立即尝试上传云端；打开应用、回到前台、网络恢复时会自动双向同步。
- 删除同步：删除会写入软删除标记，并通过先拉云端再上传最新状态的双向同步避免另一台旧设备把记录重新同步回来。
...
手动“双向同步”按钮的语义不是单纯 Git pull，也不是单纯 Git push。它会先从云端拉取记录，按 `updatedAt` 选择每条记录的最新版本，再把合并后的最新状态上传回云端。这样一台设备删除记录后，另一台旧设备同步时不会用旧数据把删除覆盖掉。
